...
int UdpPoint_SendRaw(UdpPoint* point, char* buf, int length, const char* remote, int port)
{
    ...
    if (c && buf && remote){
        ...
        ret = (-1 != (sendto(c->fd,buf,length,0,reinterpret_cast<const sockaddr * >(&addr),len)));
    }
...
Message* UdpPoint_RecvMsg(UdpPoint* point, char* remote, int* port)
{
    ...
    if (c && remote && port){
        ...
        int length ( recvfrom(c->fd, g_temp, sizeof(g_temp), //窥探一下当前有多多少数据
                    MSG_PEEK, reinterpret_cast<struct sockaddr*>(&raddr), &addrlen) );

        char* buf {(length > 0) ? reinterpret_cast<char *>(malloc(length)) : NULL};
